...
> 横断的関心事の主要な入出力パターン（正常系2件・異常系3件）

### E-1: 認証セッション検証（正常系）

| 項目 | 値 |
|------|-----|
| 入力 | Cookie: better-auth.session_token=xxx, リクエスト: GET /api/v1/events |
| 出力 | セッション有効→ユーザー情報をコンテキストに注入→APIハンドラ実行 |

### E-2: AI会話リクエスト（正常系）

| 項目 | 値 |
|------|-----|
| 入力 | POST /api/v1/ai/chat, body: { message: "イベントの準備は？" }, ロール: organizer |
| 出力 | SSEストリーミング応答、トークン数記録、会話履歴保存 |

### E-3: セッション切れ（異常系）

| 項目 | 値 |
|------|-----|
| 入力 | Cookie: 期限切れセッション, リクエスト: GET /api/v1/events |
| 出力 | 401 Unauthorized, { error: { code: "UNAUTHORIZED", message: "認証が必要です" } } |

### E-4: AI APIタイムアウト（異常系）

| 項目 | 値 |
|------|-----|
| 入力 | POST /api/v1/ai/chat（LLM API が30秒以内に応答しない） |
| 出力 | SSE error イベント { type: "error", code: "AI_TIMEOUT", message: "AIの応答がタイムアウトしました" } |

### E-5: レート制限超過（異常系）

| 項目 | 値 |
|------|-----|
| 入力 | POST /api/v1/ai/chat（1分間で21回目のリクエスト） |
| 出力 | 429 RATE_LIMITED, { error: { code: "RATE_LIMITED", message: "リクエスト頻度が高すぎます" }, retry_after: 30 } |
...
| データ項目 | 下限 | 上限 | 境界パターン |
|-----------|------|------|------------|
| セッション有効期限 | 0秒（期限切れ） | 7日（デフォルト） | 期限切れ→401、期限内→有効 |
| レート制限（AI API） | 1回/分 | 20回/分 | 20回→許可、21回→429 |
| レート制限（認証API） | 1回/分 | 10回/分 | 10回→許可、11回→429 |
| AIリクエストタイムアウト | 1ms | 30,000ms | 30秒超→タイムアウトエラー |
| ログエントリサイズ | 1 byte | 10KB | 10KB超→切り詰め |
| Zodバリデーション文字列 | 各スキーマのmin | 各スキーマのmax | 範囲外→VALIDATION_ERROR |
| CSRF Originヘッダ | 許可オリジン1つ | 許可オリジンリスト | 不一致→403 |

---

## §3-G. 例外応答 [CONTRACT]

| エラーケース | HTTPステータス | エラーコード | ユーザーへの案内 |
|------------|--------------|------------|---------------|
| Zodバリデーション失敗 | 400 | VALIDATION_ERROR | フォームに具体的なフィールドエラーを表示 |
| セッション切れ | 401 | UNAUTHORIZED | ログイン画面へリダイレクト |
| ロール権限不足 | 403 | FORBIDDEN | 「この操作には権限が必要です」トースト表示 |
| CSRF検証失敗 | 403 | CSRF_ERROR | 「ページを再読み込みしてください」 |
| AI APIタイムアウト | 200（SSE内error） | AI_TIMEOUT | 「AIの応答がタイムアウトしました。再度お試しください」 |
| AI APIレート制限 | 429 | RATE_LIMITED | 「しばらく待ってから再度お試しください」 |
| DB接続エラー | 500 | INTERNAL_ERROR | 「サーバーエラーが発生しました」+自動リトライ |
| 未捕捉例外 | 500 | INTERNAL_ERROR | 「予期しないエラーが発生しました」+エラーIDを表示 |
...
```gherkin
Scenario: Cookie認証によるAPI保護
  Given ユーザーが正常にログインしている
  When API エンドポイントにリクエストする
  Then Better Auth セッションCookieが検証される
  And ユーザー情報がリクエストコンテキストに注入される

Scenario: テナント分離の横断的保証
  Given ユーザーがテナントAに所属している
  When テナントBのリソースにアクセスする
  Then 404 Not Found が返される（存在しないものとして扱う）

Scenario: Zodバリデーションの二重チェック
  Given フロントエンドでバリデーションをパスしたデータがある
  When バックエンドAPIに送信する
  Then バックエンドでも同一のZodスキーマでバリデーションされる

Scenario: AIレート制限の適用
  Given ユーザーが認証済みである
  When 1分間にAI APIを20回呼び出す
  Then 20回目までは正常に処理される
  And 21回目は429 RATE_LIMITEDが返される

Scenario: エラーログの記録
  Given APIで500エラーが発生した
  When エラーハンドラが実行される
  Then エラーログにリクエストID・テナントID・ユーザーIDが記録される
  And ユーザーにはエラーIDのみが表示される

Scenario: CSRF対策の検証
  Given ユーザーがログイン済みである
  When 異なるオリジンからPOSTリクエストが送信される
  Then Originヘッダの不一致により403が返される
```
