# SSOT-3: API規約（API Contract） [CONTRACT]
...
- APIバージョンはURLパスに含める（`/api/v1/...`）（MUST）
- 破壊的変更時はメジャーバージョンを上げる（MUST）
- 非破壊的変更（フィールド追加等）は同バージョン内で対応可能（MAY）

---

## §7. 認証・認可 [CORE]

- 認証はBetter AuthのCookieセッションを使用する（MUST）
- 公開エンドポイント（/api/auth/*, /api/v1/invitations/*）以外は認証必須（MUST）
- テナント分離は全エンドポイントで保証する（MUST）
- ロールベースのアクセス制御をAPIレベルで実装する（MUST）
...
|----------|------|-------------|
| 統合テスト | 全エンドポイント | 70%+ |
| バリデーションテスト | リクエスト/レスポンス | 全パターン |
| エラーハンドリング | 異常系 | 全エラーコード |

- 全エンドポイントの正常系・異常系テストを実装すること（MUST）
- 認証・認可のテストを必ず含めること（MUST）

---

## §9. ドキュメント方針 [DETAIL]

- 各エンドポイントにリクエスト/レスポンス例を記載すること（SHOULD）
- エラーレスポンスの一覧を各エンドポイントに記載すること（SHOULD）

---

## §10. CORS設定 [DETAIL]

- 許可オリジンは環境変数で設定する（MUST）
- 本番環境ではワイルドカード（*）を禁止する（MUST NOT）

---

## §11. 実装優先順位 [DETAIL]

1. Phase 1（MVP）: 認証API + イベントCRUD + タスク管理 + AI チャット
2. Phase 1.5: 参加者ポータルAPI + QRチェックイン + レポート生成
3. Phase 2: MA/CRM連携API + 高度な検索・フィルタ
...
| ID | 課題 | 影響範囲 | 担当 |
|----|------|---------|------|
| API-OQ-001 | WebSocket vs SSE for realtime notifications | 通知系API | 技術 |
| API-OQ-002 | ファイルアップロードの最大サイズ | 登壇者フォーム | プロダクト |
